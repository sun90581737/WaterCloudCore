...
namespace WaterCloud.Service.CostAnalysis
{
    public class FACostByDepartmentService : RepositoryBase, IFACostByDepartmentService
    {
        ...
        public async Task<List<FACostByDepartmentEntity>> GetTableFieldList(string GetTime)
        {
            ...
            {
                strSql.Append(@"SELECT Name,SUM(Cost) Cost FROM	Sys_FACostByDepartment WHERE IsEffective=1 GROUP BY Name ");
            ...
            else
            {
                ...
                strSql.Append(@"SELECT Name, SUM(Cost) Cost FROM Sys_FACostByDepartment WHERE IsEffective=1 AND AcctDate between @starttime AND @endtime GROUP BY Name");
